...
tnets_all <- read_csv("site/totalnetsall.csv")
...
tnets_3y <- read_csv("site/totalnets_3y_all.csv")
...
```{r itn-access-trends}
accessb <- read_csv("site/access_over_time_best.csv")
...
accessa <- read_csv("site/access_over_time_all.csv")
...
pyp <- read_csv("site/pyp_all.csv")

# probably best if we drop scenario 4 and 5 from these:

pyp %>%
  filter(iso3=="MWI" & group %in% c(1, 2, 3, 6)) %>%
  ggplot(aes(
    x = pyp / 1000000,
    y = totalnets / 1000000,
...
```{r}
recc23 <- read_csv("site/table_q_s2s3long.csv") %>% 
...
```{r}
half <- read_csv("site/half_year_quantifiers.csv") %>% 
  clean_names() 
...
```{r}
gall_tnets <- read_csv("site/gall_tnets.csv") %>% 
